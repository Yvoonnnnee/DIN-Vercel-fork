...
type VerifyStatusPollerProps = {
  initialStatus: string;
};

export function VerifyStatusPoller({ initialStatus }: VerifyStatusPollerProps) {
  ...
  function handleRetry() {
    ...
    startTransition(async () => {
      ...
      if (!response.ok) {
        setError(result.error?.message || "Failed to restart verification.");
        return;
      }
      ...
      <div className="w-full max-w-md space-y-6 rounded-[28px] border border-black/5 bg-white/88 p-8 shadow-[0_24px_80px_rgba(17,24,39,0.08)] backdrop-blur">
        {status === "verified" || redirecting ? (
          <div className="space-y-4 text-center">
            ...
            <h1 className="text-2xl font-semibold tracking-tight text-ink">Verified!</h1>
            <p className="text-sm text-slate-500">Your identity has been verified. Redirecting...</p>
          </div>
        ) : status === "pending" ? (
          <div className="space-y-4 text-center">
        ...
        {status === "not_started" ? (
          <button
            type="button"
            onClick={() => router.push("/verify/start" as Parameters<typeof router.push>[0])}
            className="w-full rounded-full bg-ink px-5 py-3 text-sm font-semibold text-white transition hover:-translate-y-0.5 hover:bg-slate-800"
          >
            Start Verification
          </button>
        ) : null}
